...
# CURRENT CONFIG
current_config = str(config_folder / "config.json")
...
config_descriptions = str(config_folder / "config_descriptions.json")
...
for conds in order_conditions.keys():
    ...
    for c in order_conditions[conds].keys():
        ...
        if len(params) == 0:
            table_components = [{"components": [comp]} for comp in table_components]
...
_configs = get_downloaded_configs()
